...
// ensureBootstrapConfig writes a minimal runtime.yaml into the mihomo config dir
// when no config file exists yet. This lets mihomo start on a fresh install
// without requiring the user to configure sources first.
func (a *App) ensureBootstrapConfig() {
	...
	p := a.cfg.Policy
	bootstrap := fmt.Sprintf(`# Bootstrap config generated by proxyd – replace via UI
mixed-port: %d
allow-lan: %v
bind-address: "%s"
log-level: %s
...
rules:
  - MATCH,DIRECT
`, p.MixedPort, p.AllowLan, p.BindAddress, p.LogLevel, p.ExternalController, p.IPv6)

	if err := os.MkdirAll(a.cfg.Mihomo.ConfigDir, 0755); err != nil {
		logx.Warn("ensureBootstrapConfig: failed to create config dir", zap.Error(err))
		return
	}
	if err := os.WriteFile(configPath, []byte(bootstrap), 0644); err != nil {
		logx.Warn("ensureBootstrapConfig: failed to write bootstrap config", zap.Error(err))
		return
	}
