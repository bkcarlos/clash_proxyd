...
支持自动选择代理
支持基础日志、状态与健康检查
不做桌面端能力，不做复杂规则可视化编辑器，不做多租户和多实例。这样范围更贴合 Server 场景，也能最大化复用 mihomo 已有能力。mihomo 的文档与项目说明都表明它具备远程 provider、HTTP RESTful API controller、iptables/网关场景支持等能力。
二、总体架构
系统采用三层结构：
1. 数据面：mihomo
负责：
...
代理组执行
自动测速与自动切换
通过 external-controller 仅监听本地地址，由管理程序调用。官方文档建议通过该接口进行控制，并支持配置监听地址、secret 和 external UI。
2. 控制面：proxyd（Go + Gin）
负责：
订阅拉取
配置解析与合并
...
日志与状态聚合
定时任务与健康检查
Gin 官方文档表明它是适合 REST API 的高性能 Go Web 框架。SQLite 在 Go 中可直接通过 database/sql 与 mattn/go-sqlite3 驱动接入。
3. 展示面：Vue 3 + Vite
负责：
登录页
状态页
...
配置页
代理页
Vue 官方站点与 Vite 官方文档都适合作为这类轻量前端的基础。
三、功能范围
1. MVP 功能
订阅管理
新增订阅 URL
...
故障转移（fallback）
负载均衡（load-balance）
mihomo 官方文档对这几种代理组都有明确说明：url-test 会按 URL 定期测试并选择低延迟节点，fallback 会在当前节点失效时按顺序选择可用节点，load-balance 支持 round-robin、consistent-hashing、sticky-sessions 策略。
运行管理
启动 mihomo
停止 mihomo
重载 mihomo
...
定时任务：robfig/cron/v3
日志：zap 或 zerolog
说明一点：go-sqlite3 是 CGO 驱动，构建时需要 CGO_ENABLED=1 且系统有 gcc。这个点要在 Ubuntu 打包时提前考虑。
前端
Vue 3
Vite
Vue Router
...
mihomo 的控制能力是这套 API 能成立的关键基础，因为其文档明确提供了 external controller 与相关代理控制接口能力。
九、自动代理策略设计
这是本项目最核心的增强点。
1. 前端只暴露四种模式
手动模式
...
- name: 自动选择
  ...
  interval: 300
  tolerance: 50
url-test 是 mihomo 官方支持的自动测速组选项。
故障转移
生成：
- name: 自动选择
  type: fallback
...
- name: 自动选择
  ...
  url: https://www.gstatic.com/generate_204
  interval: 300
load-balance 的策略包括 round-robin、consistent-hashing、sticky-sessions。
3. 智能增强规则
后端可以额外增加一些简单规则：
节点数 < 3 时不生成复杂组
节点数 > 20 时自动按区域生成子组
...
external-ui 不启用
日志级别与端口按系统配置生成
mihomo 文档明确说明 external-controller 可以绑定到本地地址，也可设置 secret；这正适合本项目把控制口收敛在本机。
十二、前端页面方案
1. 登录页
字段：
用户名
...
之所以建议先用 root，是因为后续可能会涉及端口、路由、iptables 或更底层网络能力；mihomo 项目本身也强调其适用于网关/Netfilter 场景。
十五、配置示例
proxyd 自身配置
server:
  listen: 0.0.0.0:8080
